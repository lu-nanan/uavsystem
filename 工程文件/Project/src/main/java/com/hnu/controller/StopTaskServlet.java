...
@WebServlet(name = "StopTaskServlet", urlPatterns = {"/taskStop-servlet"})
public class StopTaskServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        int i = taskInfoService.updateTaskStatByTId("已停止",tid);
        int j = deviceStateService.updateActionById("空闲中",taskInfo.getDeviceid());
        if (i>0&&j>0){
            String history="用户"+userid+"暂停任务"+tid+" "+taskInfo.getContent();
            userHistoryService.add(history,userid);
            resp.getWriter().write(new JSONObject().put("result", "200").put("message", "暂停成功").toString());
            resp.sendRedirect("/UVASystem_war_exploded/html/taskList.html");
        }
        else {
            logService.addLogToFile(userid,"停止任务失败","error" ,"StopTaskServlet");
            resp.getWriter().write(new JSONObject().put("result", "error").put("message", "暂停失败").toString());
            resp.sendRedirect("/UVASystem_war_exploded/html/taskList.html");
        }
